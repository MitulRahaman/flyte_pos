					...
					@foreach($gateways as $k => $v)
                            @if ($v == 'Flytepay') 
    							...
    							<div id="paymentdiv_{{$k}}">
    								<form class="form-inline" action="{{route('subscription.flytepay')}}" method="post">
    								    @csrf
    								     <div class="form-group">
                                            <input type="number" class="form-control" style="margin-right: 2rem;" id="mobileNumber" name="mobileNumber" placeholder="Enter your mobile number" required>
                                            <input type="hidden" id="package" name="package" value="{{ request()->segment(2) }}">
