...
class SubscriptionController extends BaseController
{
    ...
    /**
     * Flyte pay payment method
     *
     * @return Response
     */
    public function pay_flytepay(Request $request)
    {
        $user = DB::table('users as u')
        ->leftJoin('business_locations as bl', function ($join) {
            $join->on('u.id', '=', 'bl.business_id');
        })
        ->where('u.id', '=', auth()->user()->id)
        ->groupBy('u.id')
        ->select('u.username', 'u.email', 'bl.state', 'bl.city', 'bl.country')
        ->first();
        
        $amount = DB::table('packages')->where('id', '=', $request->package)->first('price');
        
        $rand = Str::random(20);
        $tran_id = $user->email . $rand;
        ...
        $data = [ 
                ...
                'cus_phone' => $request->mobileNumber,
                'cus_country' => 'Bangladesh',
                'cus_email' => $user->email,
                'tran_type' => env('TRANSACTION_TYPE'),
                
            ];
            
        $apiEndpoint = 'http://flytepay.com/api/transaction';
        $client = new Client();
        try{
            $response = $client->post($apiEndpoint, [
                ...
                'headers' => [
                    ...
                    'App-key' => env('FLYTE_APP_KEY'),
                    'Tran-Type' => 'Online',
                ],
            ]);
            
            $responseArray = json_decode($response->getBody()->getContents(), true);
            return redirect()->away($responseArray['data']['redirectGatewayURL']);
            
        } catch (\Exception $e) {
            return redirect()->back()->with('error', 'Unable to payment');
        }
    }
    
    public function statusSuccess()
    {
        return view('superadmin::flytepay_transaction_status.success');
    }
    
    public function statusFailed()
    {
        return view('superadmin::flytepay_transaction_status.failed');
    }
    
    public function statusCancel()
    {
        return view('superadmin::flytepay_transaction_status.cancel');
    }
    
    public function statusIpn()
    {
        return view('superadmin::flytepay_transaction_status.ipn');
    }
